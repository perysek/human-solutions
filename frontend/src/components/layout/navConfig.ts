...
// Absence management (ABSENCE_MANAGEMENT_PLAN.md) — calendar/away glyph.
const ICON_ABSENCES =
  'M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0V11.25a2.25 2.25 0 012.25-2.25h13.5a2.25 2.25 0 012.25 2.25v7.5m-6.75-4.5l-2.25 2.25-1.5-1.5';
...
export const NAV_SECTIONS: NavSectionConfig[] = [
  {
    ...
    links: [
      ...
      },
      {
        ...
        to: '/absences',
        iconPath: ICON_ABSENCES,
        // Everyone with 'absences' access (every role, own_data — RBAC seed
        // ab01absc0003) can submit/view/cancel their own requests here.
        visible: (ctx) => ctx.hasModuleAccess('absences'),
      },
      {
        label: 'Zarządzanie nieobecnościami',
        to: '/absences/management',
        iconPath: ICON_ABSENCES,
        // Visible to everyone with 'absences' access — the page itself
        // renders a clear "no permission" message for a non-admin who isn't
        // registered as anyone's approver (absence_management_required),
        // rather than hiding the link for someone who might become an
        // approver later without a frontend redeploy.
        visible: (ctx) => ctx.hasModuleAccess('absences'),
      },
      {
        label: 'Kategorie nieobecności',
        to: '/absences/categories',
        iconPath: ICON_ABSENCES,
        mobileHide: true,
        visible: (ctx) => ctx.user.role === 'superadmin' || ctx.user.role === 'hr_manager',
      },
    ],
  },
  {
